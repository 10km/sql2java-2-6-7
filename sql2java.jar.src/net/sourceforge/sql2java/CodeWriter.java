...
public class CodeWriter {
	...
	private void writeTable(Table currentTable) throws Exception {
		...
		current_vc = new VelocityContext(vc);
		table = currentTable;
		current_vc.put("table", currentTable);
	...
	public void writeComponent(String templateName) throws Exception {
		try {
			...
			System.err.println(
					"Aborted writing component:" + templateName + (table == null ? "" : " for table:" + table.getName())
		...
		Velocity.mergeTemplate(templateName, "ISO-8859-1", current_vc, sw);
		System.out.println(" .... writing to " + current_fullfilename);
	...
	public void setCurrentFilename(String relpath_or_package, String fn) throws Exception {
		...
		current_fullfilename = destDir + File.separatorChar + relpath_or_package.replace('.', File.separatorChar)
	...
	public static String[] getExplodedString(String value) {
		...
		if (value == null)
			return new String[0];
		for (StringTokenizer st = new StringTokenizer(value, " ,;\t"); st.hasMoreTokens(); al
	...
	public static boolean authorizeFile(String folder, String schemaOrTable) {
		...
		if (include.length != 0)
			return folderContainsPattern(folder, include);
		if (exclude.length != 0)
			return !folderContainsPattern(folder, exclude);
		else
			return true;
	}
